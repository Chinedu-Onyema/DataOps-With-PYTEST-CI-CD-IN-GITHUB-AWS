...
def clean_vin_column(auction_results):
    auction_results['VIN'].replace({'Used': 'Used_Vehicle', 'Accidented': 'Used_Vehicle', '0KG': 'Used_Items_In_Container', '1,020KG': 'Used_items_In_Container', '19,958KG': 'Used_Items_In_Container'})
    return auction_results
...
def clean_transmission_column(auction_results):
    auction_results['Transmission'].replace({'AUTOMATIC': 'Automatic_Vehicle', 'Automatic': 'Automatic_Vehicle', 'Manual': 'Manual_Vehicle'})
...
def clean_fuel_type_column(auction_results):
    ...
    auction_results.at[14, 'Fuel Type']= 'Diesel_Vehicle' # Used indexing starting from 0 replacing values at specified rows
...
def clean_the_overall_table(auction_results):
    auction_results.rename(columns={'URL':'Bidding_Item', 'Reference Number':'Reference_Number', 'Price':'Bid_Winning_Price', 'Car Title':'Bidding_Item_Title', 'Condition':'Bid_Item_Identification_Number', 'VIN':'Condition', 'Transmission':'Transmission_Type', 'Fuel Type':'Fuel_Type'}, inplace=True)
...
def clean_the_price_column(auction_results):
    auction_results['Bid_Winning_Price'] = auction_results['Bid_Winning_Price'].replace({'₦': '', ',': ''}, regex=True).astype(float)
    return auction_results
...
def save_the_cleaned_table(auction_results):
    auction_results.to_csv('C:\\Users\\HP\\Desktop\\Phyton for everybody with Dr Chuck\\Scrapped_Data\\Cleaned_Scrapped_bidding_data.csv', index=False)
    print('Table saved')
    return auction_results